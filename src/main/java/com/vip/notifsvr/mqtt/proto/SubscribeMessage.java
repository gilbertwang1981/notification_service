...
public class SubscribeMessage extends RetryableMessage {
    ...
    protected int messageLength() {
        ...
        for (String topic : topics) {
            length += NsUtil.toMQttString(topic).length;
            length += 1; // topic QoS
        }
        return length;
    }
    ...
    protected void readMessage(InputStream in, int msgLength)
        ...
        while (pos < msgLength) {
            ...
            pos += NsUtil.toMQttString(topics.get(topics.size() - 1)).length;
    ...
    public List<String> getTopics() {
        return new ArrayList<String>(topics); //2016-07-14 ron.liu topics --> new ArrayList<>(topics)
    }

    public List<QoS> getTopicQoSs() {
        return new ArrayList<QoS>(topicQoSs);//2016-07-14 ron.liu topicQoSs --> new ArrayList<>(topicQoSs)
    }
